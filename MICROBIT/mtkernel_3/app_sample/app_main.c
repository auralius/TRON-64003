 ...
 * μT-Kernel 3.0
 *
 * RTOS STAGE 1.2 - N/M MODEL + TIMING
 * ------------
 ...
 *   2) LCD task       priority 9  - owns background LCD init + drawing
 *   3) INFERENCE task priority 10 - owns Noodle inference + decode/NMS
 *   4) LED task       priority 11 - heartbeat during LCD cold start
 *
 * Synchronization:
...
/* Runtime state exposed across tasks. */
static volatile INT g_lcd_ready = 0;
...
/* ------------------------------------------------------------
 * Very small status LED heartbeat using one pixel of the built-in
 * 5x5 LED matrix. This is intentionally independent of the Waveshare
 * LCD so we can show that μT-Kernel tasks are alive even while the
 * LCD task is waiting through its long cold-start settle interval.
 *
 * micro:bit V2 matrix pins (single pixel):
 *   ROW1 -> P0.21
 *   COL1 -> P0.28
 *
 * A matrix LED is lit by driving the row high and the column low.
 * ------------------------------------------------------------ */
#define APP_GPIO0_BASE         0x50000000u
#define APP_GPIO_OUTSET        0x508u
#define APP_GPIO_OUTCLR        0x50Cu
#define APP_GPIO_DIRSET        0x518u
#define APP_REG32(addr) (*(volatile uint32_t *)(uintptr_t)(addr))
#define PIN_LED_ROW_P0         21u
#define PIN_LED_COL_P0         28u

static inline void app_p0_hi(uint32_t pin)
{
    APP_REG32(APP_GPIO0_BASE + APP_GPIO_OUTSET) = (1u << pin);
}

static inline void app_p0_lo(uint32_t pin)
{
    APP_REG32(APP_GPIO0_BASE + APP_GPIO_OUTCLR) = (1u << pin);
}

static void status_led_init(void)
{
    APP_REG32(APP_GPIO0_BASE + APP_GPIO_DIRSET) =
        (1u << PIN_LED_ROW_P0) |
        (1u << PIN_LED_COL_P0);

    /* OFF state: row low, column high. */
    app_p0_lo(PIN_LED_ROW_P0);
    app_p0_hi(PIN_LED_COL_P0);
}

static void status_led_set(INT on)
{
    if (on) {
        app_p0_hi(PIN_LED_ROW_P0);
        app_p0_lo(PIN_LED_COL_P0);
    } else {
        app_p0_lo(PIN_LED_ROW_P0);
        app_p0_hi(PIN_LED_COL_P0);
    }
...
{
    /* Legacy 32-byte reply retained for the original uploader. */
    UB reply[32];
...
/*
 * New N/M-aware model information reply used by Studio Serial Upload.
 *
 * Command: MIN2
 * Reply (40 bytes):
 *   0..3   = "MIN2"
 *   4      = active slot ('A'/'B', 0 if none)
 *   5      = CUTE package format version
 *   6      = hybrid depth H
 *   7      = head depth M (0 for linear head)
 *   8..9   = layer_count, little endian
 *   10..11 = header flags, little endian
 *   12..15 = total package bytes, little endian
 *   16..39 = NUL-terminated 24-byte model label
 */
static void handle_model_info_v2(ID dd)
{
    UB reply[40];
    ...
    reply[4] = (UB)cute_model_active_slot();

    if (cute_model_ready()) {
        const CuteModelHeaderBase *h = cute_model_header();

        reply[5] = (UB)cute_model_format_version();
        reply[6] = (UB)cute_model_hybrid_blocks();
        reply[7] = (UB)cute_model_head_hidden_layers();

        const uint16_t layers = cute_model_layer_count();
        reply[8] = (UB)(layers & 0xFFu);
        reply[9] = (UB)((layers >> 8) & 0xFFu);

        if (h) {
            reply[10] = (UB)(h->flags & 0xFFu);
            reply[11] = (UB)((h->flags >> 8) & 0xFFu);
        }

        store_u32_le(&reply[12], cute_model_total_bytes());

        const char *label = cute_model_label();
        for (uint32_t i = 0;
             i < 23u && label[i];
             ++i) {
            reply[16 + i] = (UB)label[i];
        }
    }
...
/*
 * Firmware/package compatibility query.  Studio sends this before MBEG so an
 * incompatible or oversized package can be rejected without erasing a slot.
 *
 * Command: MCAP
 * Reply (16 bytes):
 *   0..3   = "MCAP"
 *   4      = maximum supported CUTE format version
 *   5      = minimum H
 *   6      = maximum H
 *   7      = maximum M (0 still denotes a linear head package)
 *   8..11  = bytes per A/B model slot
 *   12..15 = supported v2 architecture ID
 */
static void handle_model_capabilities(ID dd)
{
    UB reply[16] = {
        'M','C','A','P',
        CUTE_MODEL_FORMAT_VERSION_V2,
        CUTE_MODEL_MIN_HYBRID_BLOCKS,
        CUTE_MODEL_MAX_HYBRID_BLOCKS,
        CUTE_MODEL_MAX_HEAD_HIDDEN_LAYERS,
        0,0,0,0,
        0,0,0,0
    };

    store_u32_le(&reply[8], CUTE_SLOT_BYTES);
    store_u32_le(&reply[12], CUTE_MODEL_V2_ARCH_ID);
 ...
 * Unlike the previous revision, the LCD is initialized here as a
 * background RT task, so the long cold-power settle delay does not
 * block the rest of the system from starting.
 * ------------------------------------------------------------ */
static void lcd_task(INT stacd, void *exinf)
{
    ...
    /* This routine contains the long tk_dly_tsk()-based cold-start delay.
       Because it runs in its own task, other application tasks can proceed
       while the LCD module settles. */
    cute_lcd_init();
 ...
 * STATUS LED TASK
 *
 * Blinks a single built-in 5x5 LED matrix pixel to show that μT-Kernel
 * scheduling continues while the LCD task is waiting through its long
 * cold-start delay.
 *
 * Before LCD readiness: fast blink.
 * After LCD readiness: slow heartbeat.
 * ------------------------------------------------------------ */
static void led_task(INT stacd, void *exinf)
{
    (void)stacd;
    (void)exinf;
    ...
    INT regression_mode)
{
    ...
    /*
     * Golden behavior when the LCD is available: show the original uint8
     * image before cute_full_flash_run() quantizes it in-place.
     *
     * If the LCD task is still in its long cold-start initialization, do
     * not block communication or inference; simply skip display for this
     * transaction.
     */
    if (!regression_mode && g_lcd_ready) {
    ...
    /*
     * If the LCD is ready, overlay the detections on the already-displayed
     * image. Otherwise, return detections to the host immediately and let
     * the LCD task continue initializing in the background.
     */
    if (g_lcd_ready) {
...
static void rx_task(INT stacd, void *exinf)
{
     ...
     */
    tm_putstring(
        (UB*)"Cute-YOLO μT-Kernel RTOS Stage 1.2 H/M\n");

    tm_putstring(
        (UB*)"Tasks: RX/COMM + INFERENCE + LCD + LED\n");
    ...
    tm_putstring(
        (UB*)"LCD init task running in background...\n");

    tm_putstring(
        (UB*)"Opening sera immediately; display may become ready later...\n");
     ...
     * MEND
     * MINF  -> legacy model info
     * MIN2  -> N/M-aware model info
     * MCAP  -> model-format / slot capabilities
    ...
    for (;;) {
        ...
        else if (rolling == 0x4D494E32u) { /* MIN2 */
            handle_model_info_v2(dd);
            rolling = 0;
        }
        else if (rolling == 0x4D434150u) { /* MCAP */
            handle_model_capabilities(dd);
            rolling = 0;
        }
...
EXPORT INT usermain(void)
{
    ...
    tm_putstring(
        (UB*)"LCD will initialize in its own RT task\n");
